...
### 出来ること
    ...
    ・インターフェースの実装            ：インターフェースクラスを集約することで実現する。
    ・多態性                            ：インターフェースを実装することで実現する。
...
## インターフェースの実装方法
### インターフェース
    上記クラスの実装方法と同様にインターフェースを実現する。
    インターフェースは、クラスと同様に2つのヘッダファイルと1つのcソースファイルで構成される。
    インターフェースは空のメソッドを定義したファイルとなっている。

### インターフェースの実装
    1. インターフェースのクラスinterfaceSample_tを、使用したいクラスに集約する。
    2. 空のメソッドの中身を実装する。
    3. 後はクラスを集約した時と同様に利用する。
<br>

## 多態性
### 多態性
    インターフェースを実装することで、オブジェクト指向の多態性っぽいものを実現できる。
