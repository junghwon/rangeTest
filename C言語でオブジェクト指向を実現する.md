...
- デバイスの集約 理由1
    IIC、UARTなどは割込み処理を行うため、割込みコールバック関数が定義される別ファイル（Config_SCI5_user.c）などから参照する必要がある。
    これらデバイスのクラスを上位クラスのクラス構造体に集約すると、上位クラスのインスタンスごと別ファイルで参照可能にする必要があるが、
    そうすると本来（Config_SCI5_user.cには）必要のない別のクラスの情報にまでアクセス出来てしまう。
    これを防ぐため、デバイスのクラスはそれを必要とする上位クラスのファイルの中にグローバル変数で宣言する。
