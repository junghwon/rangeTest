...
### 割込みを必要とするデバイスの集約
IIC、UARTなどのデバイスのクラスはクラス構造体に集約せず、上位クラスのファイルの中にグローバル変数で宣言する。
（又は、シングルトンクラスで実装する。※シングルトンの実装方法は./singletonフォルダのサンプルプログラムを参照）

- デバイスの集約 理由1
    IIC、UARTなどは割込み処理を行うため、割込みコールバック関数が定義される別ファイル（Config_SCI_user.c）などから参照する必要がある。
    これらデバイスのクラスを上位クラスのクラス構造体に集約すると、上位クラスのインスタンスごと別ファイルで参照可能にする必要があるが、
    そうすると本来（Config_SCI_user.cには）必要のない別のクラスの情報にまでアクセス出来てしまう。
    これを防ぐため、デバイスのクラスはそれを必要とする上位クラスのファイルの中にグローバル変数で宣言し、
    Config_SCI_user.cなどからはこのグローバル変数を参照する。

- デバイスの集約 理由2（これは実使用上ない）
    IIC、UARTなどのデバイス（システムの中にただ一つしかないオブジェクト）をクラス構造体に集約すると、
    オブジェクトを複数宣言したときに、該当デバイスのクラスが複数実体化される可能性がある。
    よって、デバイスのオブジェクトはクラス構造体に集約せず、上位クラスのファイルの中にグローバル変数で宣言する。
    こうすることで、オブジェクトを複数宣言しても、デバイスの実態はシステム内でただ一つの存在にできる。
...
- [MVCモデルについて](https://qiita.com/s_emoto/items/975cc38a3e0de462966a)<br>
- [デザインパターン「Singleton」](https://qiita.com/shoheiyokoyama/items/c16fd547a77773c0ccc1)<br>
    https://jp-seemore.com/iot/24078/
    https://ittrip.xyz/c/c-singleton-pattern
    https://af-e.net/c-language-static-structure/#rtoc-2